...
export function getAirportInfo(code: string): AirportInfo {
  try {
    ...
    if (!airport) {
      // Fallback: return code with uppercase formatting
      return {
...
/**
 * Get city name by airport code
 * @deprecated Use getAirportCity from airport.ts instead for real airport data
 * @param code Airport IATA code
 * @returns City name
 */
export function getDestinationCityName(code: string): string {
